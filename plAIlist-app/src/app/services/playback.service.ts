...
export class PlaybackService {
  ...
  async initPlayer(getOAuthToken: GetTokenFn): Promise<void> {
    ...
    this.player = new Spotify.Player({
      ...
      getOAuthToken: (callback) => {
        getOAuthToken().then(callback);
      }
    });

    this.player.addListener('ready', ({ device_id }) => this.deviceIdSubject.next(device_id));
    ...
    this.player.addListener('account_error', ({ message }) => console.error('Spotify account error (requiere Premium):', message));
  ...
  // Espera a que el device del SDK esté listo y le pide al backend iniciar la reproducción.
